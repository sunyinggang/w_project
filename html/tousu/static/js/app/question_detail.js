...
$(function()
{
	...
	else
	{
		...
		if ($('.aw-upload-box').length)
		{
			if (G_ADVANCED_EDITOR_ENABLE == 'Y')
			{
				var fileupload = new FileUpload('file', 'http://www.tousuyun.com/static/js/app/.aw-upload-box .btn', '.aw-upload-box .upload-container', G_BASE_URL + '/publish/ajax/attach_upload/id-' + ANSWER_TYPE + '__attach_access_key-' + ATTACH_ACCESS_KEY, {
					'editor' : EDITOR
				});
			}
			else
    ...
    {
    	//邀请上一页
	    $('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .prev').click(function()
	    {
	    	if (!$(this).hasClass('active'))
	    	{
	    		...
	    		if (flag <= 0)
				{
					$('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .prev').addClass('active');
				}

		    	$('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .next').removeClass('active');
	    	}
	    });

	    //邀请下一页
	    $('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .next').click(function()
	    {
	    	if (!$(this).hasClass('active'))
	    	{
	    		var flag = 0, list = $('.aw-question-detail .aw-invite-box ul li');
	    		...
	    		for (var i = 0; i < 4; i++)
	    		{
	    			if (flag + 1 <= list.length)
	    			{
	    				...
	    				if (flag + 1 == list.length)
	    				{
	    					$('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .next').addClass('active');
	    				}
	    			}
	    		}

		 		$('http://www.tousuyun.com/static/js/app/.aw-question-detail .aw-invite-box .prev').removeClass('active');
...
function one_click_add_topic(selector, topic_title, question_id)
{
	$.post(G_BASE_URL + '/topic/ajax/save_topic_relation/', 'type=question&item_id=' + question_id + '&topic_title=' + topic_title, function (result) {
		...
		{
			$('.aw-topic-bar .tag-bar').prepend('<span class="topic-tag" data-id="' + result.rsm.topic_id + '"><a class="text" href="http://www.tousuyun.com/static/js/app/topic/' + result.rsm.topic_id + '">' + topic_title + '</a></a></span>').hide().fadeIn();
