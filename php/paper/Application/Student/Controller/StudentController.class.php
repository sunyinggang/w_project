...
class StudentController extends BaseController {
	...
	*/
	...
	public function page_select_title() {
		...
		if($state == "未选题" ) {
			if ($topic_state['topic_state1']=="教师还未批" && $topic_state['topic_state2']== "系主任还未批") {
				$this -> assign('topic_state', "还没选题，请选题");
			}
			elseif($topic_state['topic_state1']=="教师未批准" || $topic_state['topic_state2']== "系主任未批准") {
		...
		elseif($state == "选题未通过") {
			...
			elseif($topic_state['topic_state1']=="教师还未批" || $topic_state['topic_state2']== "系主任还未批" ) {
				$this -> assign('topic_state', "系统以开始审核，不可修改选题");
			}			
	...
	public function remove_selected() {
		...
		if(M('paper_student')->save($student_title)) {
			$this -> success('取消选题成功请再次选题！');
		}
		else {
			$this -> error('');
		}
...
*/
	public function title_create() {
		...
		if (M('paper_name')->query("select * from paper_name where name='".I('topic')."'")) {
			$this -> error('已经有此题目');
		}
		elseif($state!='未选题') {
			$this -> error('当前状态不可选题');
		}
		elseif(M('paper_student') -> save($student_topic)) {
			$this -> success('自拟题目成功，请等待选题通过！');
		}
	}
	/*
	学生修改个人信息和密码
	...
	*/
	...
	public function demo1() {
		$this -> display('Student/Public/top');
	}
	/*
	进行上传操作
	*/
