...
class StudentController extends BaseController {
    ...
    public function auditingtitle(){
      ...
      $data['sid'] = $leader['sid'];
    	...
    	$title = $student->selectByDept($p,10,$leader['sid']);
    	$this->assign('students',$title);
    	$count = $student->where($data)->count();
    	$Page = $this->getPages($count,10);
...
*/
        ...
        $new['topic_state2'] = $status;
           ...
           if($keyid['keyid']){
                if(($students['topic_state1']==1)&&($students['topic_state2']==1)){
                     //$this->success('操作成功',U('Wx/Index/sendTemplateMsg1',array('keyidd' => $keyid['keyid'])));
                     $this->redirect('Wx/Index/sendTemplateMsg1',array('keyidd' => $keyid['keyid']));
                     
                }else if(($students['topic_state1']==0)&&($students['topic_state2']==1)){
        ...
        /*             $Article = A('Wx://Controller/IndexController');
                ...
                }else if(($students['topic_state1']==1)&&($students['topic_state2']==2)){
                     //$this->success('操作成功',U('Wx/Index/sendTemplateMsg',array('keyidd' => $keyid['keyid'])));
                    $this->redirect('Wx/Index/sendTemplateMsg3',array('keyidd' => $keyid['keyid']));

                    
                }else if(($students['topic_state1']==0)&&($students['topic_state2']==2)){
                      //$this->success('操作成功',U('Wx/Index/sendTemplateMsg',array('keyidd' => $keyid['keyid'])));
                      $this->redirect('Wx/Index/sendTemplateMsg3',array('keyidd' => $keyid['keyid']));
                }
   ...
   public function download(){
            ...
            $leader = session('Leader');     
            $data['sid'] = $leader['sid'];
            $students = $student->selectByDept($p,10,$leader['sid']);
   ...
   //总成绩计算
    public function gradeadd(){
      ...
      $score1rule = $_POST['score1'];
        ...
        $students = $student->selectByDept2($leader['sid']);
            $count = $student->where($data)->count();
            for($i=0;$i<$count;$i++){
          ...
          // if($res){
          //   return $this->success('总成绩更新成功',U('student/look'));
          // }
